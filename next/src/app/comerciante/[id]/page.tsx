...
export default function PaginaInicio({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchComerciante = async () => {
      try {
        const respuesta = await fetch(`http://localhost:4000/api/Comerciantes/${id}`, {
          credentials: 'include',
          headers: {
        ...
        const data: Comerciante = await respuesta.json();
        data.FechaRegistro = new Date(data.FechaRegistro).toISOString().split('T')[0];
        setFormData(data);
      } catch (error) {
        console.error(error);
      }
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select 
                ...
                required
              >
                <option value="ACTIVO" selected={formData.Estado === 'ACTIVO'}>Activo</option>
                <option value="INACTIVO" selected={formData.Estado === 'INACTIVO'}>Inactivo</option>
              </select>
            </div>
            {/* <div>
